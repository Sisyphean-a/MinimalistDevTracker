...
function buildInlineScript(mode) {
  return [
    ...
    '    series: [{ type: "bar", data: items.map(function(item){ return item.value; }), itemStyle: { color: "#0f7b62" }, label: { show: true, position: "top", formatter: function(params){ const value = params && params.value != null ? params.value : 0; return unit === "hours" ? Number(value).toFixed(2) : Number(value).toLocaleString("zh-CN"); } } }]',
    '  });',
    '  const pieData = toPieData(items);',
    '  mount(pieId, {',
    '    legend: { type: "scroll", orient: "vertical", top: "middle", right: 8, itemWidth: 12, itemHeight: 10, itemGap: 8 },',
    '    tooltip: { trigger: "item", valueFormatter: function(v){ return unit === "hours" ? Number(v || 0).toFixed(2) + " 小时" : Number(v || 0).toLocaleString("zh-CN"); } },',
    '    series: [{ type: "pie", left: 8, right: 150, top: 12, bottom: 12, radius: ["34%","56%"], center: ["46%","50%"], avoidLabelOverlap: true, minShowLabelAngle: 1, label: { show: true, position: "outside", alignTo: "labelLine", distanceToLabelLine: 3, formatter: "{b} {d}%", fontSize: 11 }, labelLine: { show: true, length: 7, length2: 6, maxSurfaceAngle: 80, smooth: false }, data: pieData }]',
    '  });',
    '}',
